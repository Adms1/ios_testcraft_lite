...
public class CircularProgressBar: CALayer {
    ...
    public var completedLabel: UILabel!
    public var progressLabel: UILabel!
    public var isUsingAnimation: Bool!
    public var strProgress = ""
    public var progress: CGFloat = 0 {
        didSet {
//            progressLabel.text = "\(complete_test)/\(total_test)"//
            progressLabel.text = "\(strProgress)%"//"\(String(format: "%.0f", progress))%"
            ...
            if progress <= 40 {
                innerTrackShapeLayer.strokeColor = GetColor.successPopupGreenColor.cgColor
            }
            else if progress > 40 && progress <= 60 {
                innerTrackShapeLayer.strokeColor = GetColor.yellow.cgColor
            }else{
                innerTrackShapeLayer.strokeColor = GetColor.successPopupGreenColor.cgColor
            }
        }
    ...
    public init(radius: CGFloat, position: CGPoint, innerTrackColor: UIColor, outerTrackColor: UIColor, lineWidth: CGFloat) {
        ...
        progressLabel.font = UIFont.boldSystemFont(ofSize: radius * 0.30)
        progressLabel.text = "0%"
